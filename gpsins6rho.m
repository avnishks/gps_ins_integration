function [sys,x0,str,ts] = gpsins6rho(t,x,u,flag,dt,p1i,p2i,p3i,p4i,p5i,p6i,p7i,p8i,p9i,p10i,p11i,p12i,vxni,vyni,vzni)
...
% GPS constelation's orbital parameters
   ...
   nM = 6; 
   x_pos_ECEF = zeros(nM,1);
   y_pos_ECEF = zeros(nM,1);
   z_pos_ECEF = zeros(nM,1);
   rho_user = zeros(nM,1);
   unit_los_x = zeros(nM,1);
   unit_los_y = zeros(nM,1);
   unit_los_z = zeros(nM,1);
   ...
   sv = zeros(12,1);
   resid_mat = zeros(nM,nM);
   K_mat = zeros(12,nM);
   H_mat = zeros(nM,12);
   R_mat = zeros(nM,nM);
   d_rho_sat = zeros(nM,1);
   Y_mat = zeros(12,12);
   y = zeros(24,1);
